...
from conautoencoder import CONAutoencoder, CONAutoencoder3
from conautoencoder import CONAutoencoder2
...
def switch(type, shape):
    if type == 1:
        return CONAutoencoder(shape)
    elif type == 2:
        return CONAutoencoder2(shape)
    elif type == 3:
        return CONAutoencoder3(shape)
    else:
        assert False, "Con Autoencoder construction error"
...
def main(type: int):
    ...
    autoencoder = switch(type, shape)
    # autoencoder = CONAutoencoder(shape)
    # autoencoder = CONAutoencoder2(shape)
    # autoencoder = CONAutoencoder3(shape)

    # autoencoder = AnomalyDetector()
    # autoencoder.build((None, shape[0], shape[1], shape[2]))
    # autoencoder.summary()
    autoencoder.summary_alt(shape)
    ...
    model_des_path = os.path.join(modelresult_path, 'con_autoencoder_saved_model')
    autoencoder.save(model_des_path)
    
    model_hist_des_path = os.path.join(modelresult_path, 'con_autoencoderhistory.bin')
...
def main_ela(type: int):
    ...
    model_des_path = os.path.join(modelresult_path, 'con_autoencoder_saved_model_ela')
    autoencoder.save(model_des_path)
    
    model_hist_des_path = os.path.join(modelresult_path, 'con_autoencoderhistory_ela.bin')
    modelresulthelper.save_history(history, model_hist_des_path)
